            ...
            &mut controllers
        {
            match event {
                MovementAction::Move(direction) => {
                    ...
                    let mut dir_final = Vec3::ZERO;
                    if direction.x > 0.0 {
                        dir_final += *transform.right() * direction.x.abs();
                    }
                    if direction.x < 0.0 {
                        dir_final += *transform.left() * direction.x.abs();
                    }
                    if direction.y > 0.0 {
                        dir_final += *transform.forward() * direction.y.abs();
                    }
                    if direction.y < 0.0 {
                        dir_final += *transform.back() * direction.y.abs();
                    }
